...
class CLTrainer():
            ...
            if self.args.poison_knn_eval_freq != 0 and epoch % self.args.poison_knn_eval_freq == 0:
                with torch.no_grad():
                    knn_acc, back_acc = self.knn_monitor_fre(
                        model.module.backbone if self.args.distributed else model.backbone, poison.memory_loader,
            ...
            if not self.args.distributed or (self.args.distributed
                ...
                if epoch % self.args.save_freq == 0:
                    save_model({
                        ...
                        'state_dict': model.state_dict(),
                        'optimizer': optimizer.state_dict(),
                    }, filename=os.path.join(self.args.saved_path, 'epoch__%s.pth.tar' % (epoch + 1)))
    ...
    def train_freq_my(self, model, optimizer,   train_transform,  poison):
        cosine_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, self.args.epochs)
        warmup_scheduler = GradualWarmupScheduler(optimizer, multiplier=1, total_epoch=self.args.warmup_epoch,
                                                  after_scheduler=cosine_scheduler)
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            for i, ((v1, v2), _) in enumerate(train_loader):  #frequency backdoor has been injected
                ...
                if self.args.method == 'simclr':
                    features = model(v1.cuda(self.args.gpu, non_blocking=True), v2.cuda(self.args.gpu, non_blocking=True))
                    loss, _, _ = model.criterion(features)
                ...
                # loss = model.loss(reps)
                losses.update(loss.item(), v1[0].size(0))
                cl_losses.update(loss.item(), v1[0].size(0))
            ...
            if self.args.poison_knn_eval_freq != 0 and epoch % self.args.poison_knn_eval_freq == 0:
                with torch.no_grad():
                    knn_acc, back_acc = self.knn_monitor_fre_my(
                        model.module.backbone if self.args.distributed else model.backbone, poison.memory_loader,
                        test_loader, epoch, self.args,
                        classes=self.args.num_classes,
                        subset=False,
    ...
    @torch.no_grad()
    def knn_monitor_fre_my(self, net, memory_data_loader, test_data_loader, epoch, args, k=200, t=0.1, hide_progress=True,
                        classes=-1, subset=False, backdoor_loader=None):
