...
class PoisonAgent_my():
    ...
    def choose_poisons_randomly(self):
        ...
        if self.args.threat_model == 'our':
                ...
                x_test_pos_tensor, y_test_pos_tensor = self.fre_poison_agent.Poison_Frequency_Diff(x_test_tensor.clone().detach(), y_test_tensor.clone().detach(), self.magnitude * 2)
...
class TensorsDataset_Offline(torch.utils.data.Dataset):
    '''
    ...
    def __init__(self, data_tensor, target_tensor=None, transforms=None):
        ...
        print("dataset length is: ", self.data_tensor.size(0))
    ...
    def __getitem__(self, item):
        img, target = self.data_tensor[item], self.target_tensor[item]
        imgs = self.transforms(img)
        return imgs, target  # stack a positive pair
