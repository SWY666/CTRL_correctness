...
from loaders.diffaugment import set_aug_diff, PoisonAgent, PoisonAgent_my
...
parser.add_argument('--poisoning', action='store_false', default=True)
...
parser.add_argument('--trial', default='test', type=str)
...
args.disable_normalize = False
args.diff_aug = True
args.poisoning = True
args.full_dataset = True
args.channel = [1, 2]
args.trigger_position = [15, 31]
...
def main_worker(gpu,  args):
    ...
    if args.poisoning:
            ...
            poison = PoisonAgent_my(args, poison_frequency_agent, train_dataset, test_dataset, memory_loader, args.magnitude)
    ...
    if args.mode == 'normal':
         ...
         trainer.train_freq_my(model, optimizer, train_transform,  poison)
